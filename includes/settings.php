...
function appsumo_edd_product_settings_field_active( $download_id, $price_id = null ) {
	...
	<span class="appsumo-edd-enabled">
		<label class="edd-legacy-setting-label"><?php esc_html_e( 'Enable', 'sales-wk-addon-edd-appsumo' ); ?></label>

		<select name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $name ); ?>">
			<option value="no" <?php selected( $active, false ); ?>><?php echo esc_attr_e( 'No', 'sales-wk-addon-edd-appsumo' ); ?></option>
			<option value="yes" <?php selected( $active, true ); ?>><?php echo esc_attr_e( 'Yes', 'sales-wk-addon-edd-appsumo' ); ?></option>
		</select>
	</span>
...
function appsumo_edd_product_settings_field_codes_num( $download_id, $price_id = null ) {
	...
	<span class="appsumo-edd-codes">
		<label class="edd-legacy-setting-label"><?php esc_html_e( 'Codes', 'sales-wk-addon-edd-appsumo' ); ?></label>
...
function appsumo_edd_product_settings_field_redeemed( $download_id, $price_id = null ) {
	...
	<span class="appsumo-edd-codes-redeemed">
		<label class="edd-legacy-setting-label"><?php esc_html_e( 'Redeemed', 'sales-wk-addon-edd-appsumo' ); ?></label>
...
function appsumo_edd_product_settings_field_remaining( $download_id, $price_id = null ) {
	...
	<span class="appsumo-edd-codes-remaining">
		<label class="edd-legacy-setting-label"><?php esc_html_e( 'Remaining', 'sales-wk-addon-edd-appsumo' ); ?></label>
...
function appsumo_edd_variable_price_settings( $download_id, $price_id, $args ) {
	...
	<div class="edd-custom-price-option-section">
		<span class="edd-custom-price-option-section-title"><?php esc_html_e( 'Appsumo Settings', 'sales-wk-addon-edd-appsumo' ); ?></span>
		...
		if ( $post->post_name ) {
			...
			$link = appsumo_get_landing_page_url();
		...
		<div class="appsumo_vp_landing_page_link">
			...
			<?php if (!$permalink_structure) { ?>
				<br /><br /><span style='color:green'>You are currently using the <b>Plain</b> permalink structure. To customize your permalinks and have URLs like <b>/appsumo/</b>, navigate to 'Settings' &gt; 'Permalinks' in your WordPress dashboard and select <b>Custom</b> permalink structure.</span>
			<?php } ?>
		</div>
...
function appsumo_edd_download_settings_save( $post_id, $post ) {
	...
	if ( edd_has_variable_prices( $post_id ) ) {
		...
		$active            = filter_input( INPUT_POST, 'appsumo_active', FILTER_SANITIZE_STRING );
		$appsumo_codes_num = filter_input( INPUT_POST, 'appsumo_codes_num', FILTER_SANITIZE_STRING );
...
function appsumo_save_download_settings( $download_id, $price_id = null ) {
	...
	if ( $appsumo_code_gen_num !== $previous_codes ) {
		...
		if ( $appsumo_code_gen_num > $previous_codes ) {
			...
			appsumo_generate_codes( $download_id, ( $appsumo_code_gen_num - $previous_codes ), $price_id );
